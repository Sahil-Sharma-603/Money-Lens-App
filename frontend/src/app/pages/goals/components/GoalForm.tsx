...
export default function GoalForm({ onClose, onSubmit }) {
  // Initial goal values with API expected field names
  ...
  const [targetDate, setTargetDate] = useState(''); // Target date input
  const [limitAmount, setLimitAmount] = useState(0);
  const [category, setCategory] = useState(''); // Category required for Savings goals
  const [interval, setInterval] = useState('Daily');
  const [error, setError] = useState(null);
  const [isLoading, setIsLoading] = useState(false);

  // Compute tomorrow's date as a string (YYYY-MM-DD)
  ...
  // Inline style to remove spinner controls (note: pseudo selectors require CSS)
  ...
  // Update sub-goal field using the index
  ...
  // Submit form data to the API with the field names the server expects
  const handleSubmit = async (e) => {
    ...
    try {
      const goalData = {
        title,            // Goal name
        targetAmount,     // Required for API as targetAmount
        targetDate,       // Required target date
        type,             // Goal type (Savings or Spending Limit)
        selectedAccount,  // Provided account (if needed)
        subGoals,
        limitAmount,
        category,         // Category now provided for Savings goals
        interval,
      };
  ...
  // Recalculate percentages for sub-goals
  const updateSubGoalPercentages = () => {
    const totalPercentage = subGoals.reduce(
      (sum, subGoal) => sum + parseFloat(subGoal.percentage || 0),
      0
    );
    const scale = totalPercentage === 100 ? 1 : totalPercentage > 0 ? 100 / totalPercentage : 1;
    const newSubGoals = subGoals.map((subGoal) => ({
      ...subGoal,
      percentage: (parseFloat(subGoal.percentage || 0) * scale).toFixed(2),
    }));
    setSubGoals(newSubGoals);
  };

  // Validate that the sum of sub-goal amounts does not exceed the total goal amount
  const handleSubGoalAmountChange = () => {
    const totalSubGoalAmount = subGoals.reduce(
      (sum, subGoal) => sum + parseFloat(subGoal.amount || 0),
      0
    );
          ...
          <div className={`${styles.savingsGoal} ${styles.formGroup}`}>
            ...
            {type === 'Savings' && (
              <>
                ...
                {/* Added Category input for Savings goals */}
